...
`;const v=window.matchMedia("(prefers-reduced-motion: reduce)"),E=document.querySelector(".entrance"),P=document.querySelector(".poem"),g=document.querySelector(".enter-button"),y=document.querySelector(".portfolio"),m=document.querySelector(".detail-view"),_=document.querySelector(".detail-view__inner"),c=document.querySelector(".back-button"),p=document.querySelector(".music-toggle"),U=document.querySelector(".audio-status"),$=document.querySelector(".cursor-glow"),z=document.querySelectorAll("[data-depth]"),f=document.querySelector(".rain-canvas"),r=f.getContext("2d"),d=new Audio(a.music);d.loop=!0;d.preload="auto";d.volume=1;d.load();let u=!1,q=!1,A=[],C=0,s={view:"portfolio",categoryId:null,personId:null};function k(e){return a.favorites.find(t=>t.id===e)||null}function j(e,t){return e?.people.find(o=>o.id===t)||null}function L(){const e=window.location.hash.replace(/^#/,""),o=(e.startsWith("/")?e.slice(1):e).split("/").filter(Boolean);if(o[0]==="favorites"&&o[1]){const n=k(o[1]);if(!n)return{view:"portfolio",categoryId:null,personId:null};if(o[2]){const i=j(n,o[2]);return i?{view:"person",categoryId:n.id,personId:i.id}:{view:"category",categoryId:n.id,personId:null}}return{view:"category",categoryId:n.id,personId:null}}return{view:"portfolio",categoryId:null,personId:null}}function x(e){const t=e.startsWith("#")?e:`#${e}`;if(window.location.hash===t){T(L());return}window.location.hash=t}function D(e){return`
  ...
  `}function h(){if(!u){c.hidden=!0;return}if(c.hidden=!1,s.view==="person"){c.setAttribute("aria-label","Back to category"),c.textContent="← Back";return}if(s.view==="category"){c.setAttribute("aria-label","Back to portfolio"),c.textContent="← Back";return}c.setAttribute("aria-label","Back to entrance"),c.textContent="← Back"}function M(e){_.innerHTML=e,m.hidden=!1,m.setAttribute("aria-hidden","false"),document.body.classList.add("has-detail"),y.setAttribute("aria-hidden","true"),window.scrollTo({top:0,behavior:"auto"}),m.focus({preventScroll:!0})}function I(){m.hidden=!0,m.setAttribute("aria-hidden","true"),_.innerHTML="",document.body.classList.remove("has-detail"),u&&y.setAttribute("aria-hidden","false")}function T(e){if(s=e,!u){I(),h();return}if(s.view==="category"){const t=k(s.categoryId);if(!t){x("#/");return}M(D(t)),h();return}if(s.view==="person"){const t=k(s.categoryId),o=j(t,s.personId);if(!t||!o){x(t?`#/favorites/${t.id}`:"#/");return}M(G(t,o)),h();return}I(),h()}function J(){if(u){if(s.view==="person"&&s.categoryId){x(`#/favorites/${s.categoryId}`);return}if(s.view==="category"){x("#/");return}K()}}function H(e,t,o=!1){const n=Math.random();return{x:Math.random()*e,y:o?Math.random()*t:-Math.random()*t*.3,length:10+n*18,speed:4.5+n*9,thickness:.7+n*1.4,opacity:.18+n*.45}}function Q(){const e=Math.min(window.devicePixelRatio||1,2),t=window.innerWidth,o=window.innerHeight;f.width=Math.floor(t*e),f.height=Math.floor(o*e),f.style.width=`${t}px`,f.style.height=`${o}px`,r.setTransform(e,0,0,e,0,0);const n=Math.round(Math.min(160,Math.max(70,t*.12)));A=Array.from({length:n},()=>H(t,o,!0))}function R(){const e=window.innerWidth,t=window.innerHeight;r.clearRect(0,0,e,t);for(const o of A){o.y+=o.speed,o.x+=o.speed*.08,(o.y>t+o.length||o.x>e+20)&&(Object.assign(o,H(e,t,!1)),o.y=-o.length),r.beginPath();const n=o.opacity>.4?"210, 220, 230":"220, 210, 200";r.strokeStyle=`rgba(${n}, ${o.opacity*.85})`,r.lineWidth=o.thickness,r.lineCap="round",r.moveTo(o.x,o.y),r.lineTo(o.x-o.length*.12,o.y+o.length),r.stroke()}C=window.requestAnimationFrame(R)}function W(){if(Q(),v.matches){r.clearRect(0,0,window.innerWidth,window.innerHeight);for(const e of A.slice(0,40))r.beginPath(),r.strokeStyle=`rgba(220, 230, 240, ${e.opacity*.7})`,r.lineWidth=e.thickness,r.moveTo(e.x,e.y),r.lineTo(e.x-e.length*.12,e.y+e.length),r.stroke();return}cancelAnimationFrame(C),R()}function w(e){U.textContent=e}function S(e){q=e,p.hidden=!1,p.textContent=e?"Sound: On":"Sound: Off",p.setAttribute("aria-label",e?"Pause music":"Play music"),p.setAttribute("aria-pressed",e?"true":"false")}async function N(){try{d.volume=1,d.muted=!1,await d.play(),S(!0),w("")}catch(e){S(!1),w("Music could not start. Tap Sound in the corner to retry."),console.error("Audio playback failed:",e)}}function O(){d.pause(),S(!1)}function X(){u=!0,document.body.classList.add("is-entered"),E.setAttribute("aria-hidden","true"),P.setAttribute("aria-hidden","true"),y.setAttribute("aria-hidden","false"),window.scrollTo({top:0,behavior:"auto"});const e=L();e.view==="portfolio"&&window.location.hash!=="#/"?x("#/"):T(e),s.view==="portfolio"&&y.focus({preventScroll:!0})}function K(){u&&(u=!1,document.body.classList.remove("is-entered"),E.setAttribute("aria-hidden","false"),P.setAttribute("aria-hidden","false"),y.setAttribute("aria-hidden","true"),I(),s={view:"portfolio",categoryId:null,personId:null},window.location.hash&&window.location.hash!=="#poem"&&history.replaceState(null,"",`${window.location.pathname}${window.location.search}`),h(),g.disabled=!1,w(""),window.scrollTo({top:0,behavior:v.matches?"auto":"smooth"}),g.focus())}async function Y(){u||(g.disabled=!0,w("Starting the room tone..."),await N(),X())}async function Z(){if(q){O();return}await N()}function ee(e){window.innerWidth<900||($.style.opacity="1",$.style.transform=`translate(${e.clientX}px, ${e.clientY}px)`)}function te(){$.style.opacity="0"}function B(){if(v.matches)return;const e=window.innerHeight||1;z.forEach(t=>{const o=Number(t.dataset.depth||0),i=(t.getBoundingClientRect().top-e*.5)*o;t.style.transform=`translate3d(0, ${i}px, 0)`})}g.addEventListener("click",Y);c.addEventListener("click",J);p.addEventListener("click",Z);window.addEventListener("hashchange",()=>{u&&T(L())});window.addEventListener("pointermove",ee);window.addEventListener("pointerleave",te);window.addEventListener("scroll",B,{passive:!0});window.addEventListener("resize",()=>{B(),W()});d.addEventListener("ended",()=>{d.loop||O()});v.matches&&document.body.classList.add("reduced-motion");h();B();W();
