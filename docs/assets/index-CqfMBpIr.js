...
`;const m=window.matchMedia("(prefers-reduced-motion: reduce)"),$=document.querySelector(".entrance"),k=document.querySelector(".poem"),u=document.querySelector(".enter-button"),y=document.querySelector(".portfolio"),v=document.querySelector(".back-button"),d=document.querySelector(".music-toggle"),R=document.querySelector(".audio-status"),b=document.querySelector(".cursor-glow"),j=document.querySelectorAll("[data-depth]"),l=document.querySelector(".rain-canvas"),a=l.getContext("2d"),c=new Audio(i.music);c.loop=!0;c.preload="auto";c.volume=1;c.load();let h=!1,S=!1,w=[],A=0;function E(e,o,t=!1){const r=Math.random();return{x:Math.random()*e,y:t?Math.random()*o:-Math.random()*o*.3,length:10+r*18,speed:4.5+r*9,thickness:.7+r*1.4,opacity:.18+r*.45}}function q(){const e=Math.min(window.devicePixelRatio||1,2),o=window.innerWidth,t=window.innerHeight;l.width=Math.floor(o*e),l.height=Math.floor(t*e),l.style.width=`${o}px`,l.style.height=`${t}px`,a.setTransform(e,0,0,e,0,0);const r=Math.round(Math.min(160,Math.max(70,o*.12)));w=Array.from({length:r},()=>E(o,t,!0))}function L(){const e=window.innerWidth,o=window.innerHeight;a.clearRect(0,0,e,o);for(const t of w){t.y+=t.speed,t.x+=t.speed*.08,(t.y>o+t.length||t.x>e+20)&&(Object.assign(t,E(e,o,!1)),t.y=-t.length),a.beginPath();const r=t.opacity>.4?"210, 220, 230":"220, 210, 200";a.strokeStyle=`rgba(${r}, ${t.opacity*.85})`,a.lineWidth=t.thickness,a.lineCap="round",a.moveTo(t.x,t.y),a.lineTo(t.x-t.length*.12,t.y+t.length),a.stroke()}A=window.requestAnimationFrame(L)}function M(){if(q(),m.matches){a.clearRect(0,0,window.innerWidth,window.innerHeight);for(const e of w.slice(0,40))a.beginPath(),a.strokeStyle=`rgba(220, 230, 240, ${e.opacity*.7})`,a.lineWidth=e.thickness,a.moveTo(e.x,e.y),a.lineTo(e.x-e.length*.12,e.y+e.length),a.stroke();return}cancelAnimationFrame(A),L()}function p(e){R.textContent=e}function g(e){S=e,d.hidden=!1,d.textContent=e?"Sound: On":"Sound: Off",d.setAttribute("aria-label",e?"Pause music":"Play music"),d.setAttribute("aria-pressed",e?"true":"false")}async function _(){try{c.volume=1,c.muted=!1,await c.play(),g(!0),p("")}catch(e){g(!1),p("Music could not start. Tap Sound in the corner to retry."),console.error("Audio playback failed:",e)}}function P(){c.pause(),g(!1)}function B(){h=!0,document.body.classList.add("is-entered"),$.setAttribute("aria-hidden","true"),k.setAttribute("aria-hidden","true"),y.setAttribute("aria-hidden","false"),v.hidden=!1,window.scrollTo({top:0,behavior:"auto"}),y.focus({preventScroll:!0})}function W(){h&&(h=!1,document.body.classList.remove("is-entered"),$.setAttribute("aria-hidden","false"),k.setAttribute("aria-hidden","false"),y.setAttribute("aria-hidden","true"),v.hidden=!0,u.disabled=!1,p(""),window.scrollTo({top:0,behavior:m.matches?"auto":"smooth"}),u.focus())}async function C(){h||(u.disabled=!0,p("Starting the room tone..."),await _(),B())}async function N(){if(S){P();return}await _()}function H(e){window.innerWidth<900||(b.style.opacity="1",b.style.transform=`translate(${e.clientX}px, ${e.clientY}px)`)}function O(){b.style.opacity="0"}function x(){if(m.matches)return;const e=window.innerHeight||1;j.forEach(o=>{const t=Number(o.dataset.depth||0),n=(o.getBoundingClientRect().top-e*.5)*t;o.style.transform=`translate3d(0, ${n}px, 0)`})}u.addEventListener("click",C);v.addEventListener("click",W);d.addEventListener("click",N);window.addEventListener("pointermove",H);window.addEventListener("pointerleave",O);window.addEventListener("scroll",x,{passive:!0});window.addEventListener("resize",()=>{x(),M()});c.addEventListener("ended",()=>{c.loop||P()});m.matches&&document.body.classList.add("reduced-motion");x();M();
